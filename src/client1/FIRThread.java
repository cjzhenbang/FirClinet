...
public class FIRThread extends Thread
{
	...
	public void dealWithMsg(String msgReceived)    // 处理取得的信息
	{
		if (msgReceived.startsWith("/chess "))   // 收到的信息为下棋
		{ 
			StringTokenizer userMsgToken = new StringTokenizer(msgReceived, " ");
			
			String[] chessInfo = { "-1", "-1", "0" };  // 表示棋子信息的数组、0索引为：x坐标；1索引位：y坐标；2索引位：棋子颜色
			int i = 0; // 标志位
		...
		else if (msgReceived.startsWith("/yourname "))   // 收到的信息为改名
		{ 
