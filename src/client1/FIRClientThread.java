...
public class FIRClientThread extends Thread
{
	...
	public void dealWithMsg(String msgReceived)
	{
		...
		{ 
			StringTokenizer userToken = new StringTokenizer(msgReceived, " ");// 若取得的信息为用户列表
			int userNumber = 0;
			
			firClient.userListPad.userList.removeAll();// 清空客户端用户列表
			
			firClient.userInputPad.userChoice.removeAllItems();
			firClient.userInputPad.userChoice.addItem("所有用户");// 给客户端用户下拉框添加一个选项
